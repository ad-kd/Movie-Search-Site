...
import '../../css/reactbits/Waves.css';
...
const Waves = ({ lineColor = 'rgba(255, 255, 255, 0.2)', backgroundColor = 'transparent', waveSpeedX = 0.0125, waveSpeedY = 0.005, waveAmpX = 32, waveAmpY = 16, xGap = 10, yGap = 32, friction = 0.925, tension = 0.005, maxCursorMove = 100, style = {}, className = '' }) => {
  ...
  useEffect(() => {
    ...
    function tick(t) {
      ...
      const d = Math.hypot(dx, dy); mouse.v = d; mouse.vs += (d - mouse.vs) * 0.1; mouse.vs = Math.min(100, mouse.vs);
      ...
      lines.forEach(pts => pts.forEach(p => {
        const move = noiseRef.current.perlin2((p.x + t * waveSpeedX) * 0.002, (p.y + t * waveSpeedY) * 0.0015) * 12;
        p.wave.x = Math.cos(move) * waveAmpX; p.wave.y = Math.sin(move) * waveAmpY;
        const dMouse = Math.hypot(p.x - mouse.sx, p.y - mouse.sy), l = Math.max(175, mouse.vs);
        if (dMouse < l) {
          const s = 1 - dMouse / l, f = Math.cos(dMouse * 0.001) * s;
          p.cursor.vx += Math.cos(mouse.a) * f * l * mouse.vs * 0.00065; p.cursor.vy += Math.sin(mouse.a) * f * l * mouse.vs * 0.00065;
        }
        p.cursor.vx = (p.cursor.vx + (0 - p.cursor.x) * tension) * friction;
        p.cursor.vy = (p.cursor.vy + (0 - p.cursor.y) * tension) * friction;
      ...
      ctx.beginPath(); ctx.strokeStyle = configRef.current.lineColor;
      lines.forEach(points => {
        const p0 = points[0];
        ctx.moveTo(p0.x + p0.wave.x, p0.y + p0.wave.y);
        points.forEach((p, idx) => {
          const isLast = idx === points.length - 1;
          const pos = { x: p.x + p.wave.x + (isLast ? 0 : p.cursor.x), y: p.y + p.wave.y + (isLast ? 0 : p.cursor.y) };
    ...
    const onMouseMove = (e) => { const b = boundingRef.current; mouseRef.current.x = e.clientX - b.left; mouseRef.current.y = e.clientY - b.top; if (!mouseRef.current.set) { mouseRef.current.sx = mouseRef.current.x; mouseRef.current.sy = mouseRef.current.y; mouseRef.current.set = true; } };
    window.addEventListener('resize', onResize); window.addEventListener('mousemove', onMouseMove);
    return () => { window.removeEventListener('resize', onResize); window.removeEventListener('mousemove', onMouseMove); cancelAnimationFrame(frameIdRef.current); };
  }, []);

  return <div ref={containerRef} className={`waves ${className}`} style={{ position: 'absolute', top: 0, left: 0, width: '100%', height: '100%', overflow: 'hidden', backgroundColor, ...style }}><canvas ref={canvasRef} className="waves-canvas" /></div>;
};
